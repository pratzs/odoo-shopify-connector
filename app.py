...
def get_settings():
    ...
    keys = ['odoo_company_id', 'inventory_field', 'sync_zero_stock', 'inventory_locations', 'prod_sync_price', 'prod_sync_title', 'prod_sync_desc', 'prod_sync_images', 'prod_auto_create']
    data = {'odoo_url': shop.odoo_url, 'odoo_db': shop.odoo_db, 'odoo_username': shop.odoo_username, 'has_password': bool(shop.odoo_password)}
    for k in keys:
    ...
    try:
        with shopify.Session.temp(shop.shop_url, '2024-01', shop.access_token):
            ...
            for o in orders:
                ...
                orders_data.append({'id': o.id, 'name': o.name, 'created_at': o.created_at, 'total': o.total_price, 'financial_status': o.financial_status, 'sync_status': status})
...
def manual_sync_order():
    ...
    try:
        ...
        with shopify.Session.temp(shop.shop_url, '2024-01', shop.access_token):
            ...
            if success: return jsonify({'message': f'Order {order.name} synced'})
            else: return jsonify({'error': 'Sync failed'}), 400
    except Exception as e: return jsonify({'error': str(e)}), 500
...
def save_settings():
    ...
    if 'odoo_url' in data:
        shop.odoo_url = data['odoo_url']; shop.odoo_db = data['odoo_db']; shop.odoo_username = data['odoo_username']
        if data.get('odoo_password'): shop.odoo_password = data['odoo_password']
        shop.odoo_company_id = data.get('odoo_company_id')
        db.session.commit()
    
    keys = ['inventory_field', 'sync_zero_stock', 'inventory_locations', 'prod_sync_price', 'prod_sync_title', 'prod_sync_desc', 'prod_sync_images', 'prod_auto_create']
    for key in keys:
    ...
    if 'odoo_username' in data:
        try:
            ...
            return jsonify({'error': f'Saved but Connection Failed: {str(e)}'}), 400
...
def process_order_data(data, shop, odoo):
    ...
    try:
        ...
        if not partner:
            ...
            vals = {'name': addr.get('company') or f"{cust.get('first_name')} {cust.get('last_name')}", 'email': email, 'phone': cust.get('phone'), 'street': addr.get('address1'), 'city': addr.get('city'), 'zip': addr.get('zip'), 'country_code': addr.get('country_code'), 'vat': vat, 'is_company': True, 'company_type': 'company'}
        ...
        def get_child(addr_data, type_val):
            ...
            return odoo.find_or_create_child_address(partner_id, {'name': name, 'street': addr_data.get('address1'), 'city': addr_data.get('city'), 'zip': addr_data.get('zip'), 'country_code': addr_data.get('country_code'), 'phone': addr_data.get('phone'), 'email': email}, type_val)
        ...
        vals = {'partner_id': partner_id, 'partner_invoice_id': invoice_id, 'partner_shipping_id': shipping_id, 'order_line': lines, 'user_id': user_id, 'note': f"Gateway: {data.get('gateway')}"}
        ...
        if existing_order_id:
            ...
            vals['name'] = client_ref; vals['client_order_ref'] = client_ref; vals['state'] = 'draft'
            if company_id: vals['company_id'] = int(company_id)
        ...
        return False
    finally: active_processing_ids.discard(shopify_id)

def process_product_data(data, shop, odoo):
    ptype = data.get('product_type', ''); cat_id = None
    if ptype:
        try:
...
def task_sync_inventory(shop, odoo):
    ...
    except Exception as e: print(f"Inventory Error {shop.shop_url}: {e}")
...
def callback():
    ...
    if not shop:
        ...
        db.session.add(shop)
    shop.access_token = token; shop.is_active = True
    db.session.commit()
    with shopify.Session.temp(shop_url, '2024-01', token):
        hooks = [{'topic': 'orders/updated', 'address': f'{APP_URL}/webhook/orders/updated'}, {'topic': 'products/update', 'address': f'{APP_URL}/webhook/products/update'}]
        for h in hooks:
            try: w = shopify.Webhook(); w.topic = h['topic']; w.address = h['address']; w.format = 'json'; w.save()
            except: pass
