...
class OdooClient:
    ...
    def search_product_by_sku(self, sku, company_id=None):
        ...
        if company_id:
            # POLISH NOTATION FIX: ['&', A, '|', B, C]  ==  A AND (B OR C)
            domain = [
                '&', 
                ['default_code', '=', sku], 
                '|', 
    ...
    def create_service_product(self, name, company_id=None):
        ...
        vals = {
            'name': name,
            'type': 'service',  # Service type so it doesn't track stock
            'invoice_policy': 'order',
            'list_price': 0.0,  # Price determined by order line
            'sale_ok': True,
            'purchase_ok': False,
        }
        if company_id:
    ...
    def get_changed_products(self, time_limit_str, company_id=None):
        ...
        if company_id:
            # Get products modified recently AND (belong to Company OR Shared)
            domain = [
                '&',
                '&', # Nested AND for the write_date/type
    ...
    def post_message(self, order_id, message):
        return self.models.execute_kw(self.db, self.uid, self.password, 'sale.order', 'message_post', [order_id], {'body': message})import xmlrpc.client
import ssl
